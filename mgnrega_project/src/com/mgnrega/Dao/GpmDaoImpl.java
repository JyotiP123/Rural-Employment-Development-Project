...
public class GpmDaoImpl implements GpmDao{
		 	...
		 	if(rs.next()) {
		 		...
		 		if(rs2.next()) {
		 			...
		 			if(x > 0)
		 				...
		 				throw new EmpException("Techical error..");
	...
	@Override
	public List<Emp> getEmployeeByMobile(String emobile) throws EmpException  {
		...
		try(Connection conn = DBUtil.provideConnection()) {
			...
			ResultSet rs= ps.executeQuery();
				...
				Emp emp1 =new Emp(id,n,m,a,bdoid,v);	
				emp.add(emp1);
		...
		if(emp.isEmpty())
			throw new EmpException("No Employee found in the Project");
